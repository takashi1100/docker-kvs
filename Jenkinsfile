pipeline {
  agent any
  environment {
    DOCKERHUB_USER = "zaki1100"
    BUILD_HOST = "root@172.20.10.2"
    PROD_HOST = "root@172.20.10.3"
    BUILD_TIMESTAMP = sh(script: "date +%Y%m%d-%H%M%S", returnStdout: true).trim()
  }
  stages {
    stage('Pre Check') {
      steps {
        sh "test -f /root/.docker/config.json"
        sh "cat /root/.docker/config.json | grep docker.io"
      }
    }
    stage('Build') {
      steps {
